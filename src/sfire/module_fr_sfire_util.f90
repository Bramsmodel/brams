            ...
            do j1 = js, je
               ...
               do i1 = is, ie
                  ...
                  v1(i1, j1) = &
                     ...
                     + tx*ty*v2(i2 + 1, j2 + 1)
                  !PEREIRA
                  write(53,*) i2,j2,i1,j1,rx,ry,rjo,rio,tx,ty,v1(i1, j1)
               end do
            end do
         end do
      end do
...
11    msg = 'Error closing file '//trim(filename)
   ...
   pure integer function snode(t, d, i) result(saida)
    ...
    integer, intent(in) ::ims, ime, kms, kme, jms, jme, & 
                         ids, ide, kds, kde, jds, jde  ! Índices principais
    real, intent(in), dimension(ims:ime, kms:kme, jms:jme) :: a
    character(len=*) :: name
    ...
    do j = jds, jde
        do k = kds, kde
            do i = ids, ide
                rel = a(i, k, j)
                lsum = ieor(lsum, iel)  ! Operação bit a bit XOR para cálculo do checksum
            end do
        end do
    end do

    ! Impressão dos resultados do checksum
    ...
    ! Inicialização de variáveis e lógica para diferentes funções
    ...
    ! Ajustes para soma, máximo e mínimo
    ...
    gsum = psum  ! Simplificação para remover paralelismo
...
!   end subroutine build_NFFL
  ...
  real, parameter :: Raio = 6372.795477598*1000.0

  integer, parameter :: rows = 24286, ncols = 16439
  integer, parameter :: escrever = 1
  real, parameter :: res_ant_init = 1000.0
  real, allocatable :: idclass(:,:)
  real, allocatable :: vlat(:), vlon(:)
  integer :: i, j, ia, ja
  real :: lat1, lon1, lat2, lon2, delta_lat, delta_lon, dist_lat, dist_lon
  real :: grid_latitude, grid_longitude, nearest_dist,res_ant,a,b,c
  integer :: recl_size
  character(len=255) :: filename
  character(len=80) :: fname
  character(len=255) :: lixo
  ...
  ! Abrir o arquivo de dados de combustível
  filename = "./data_comb/comb_port1.txt"
  inquire(file=trim(filename), exist=file_exists)
  if (.not. file_exists) then
    print *, "Arquivo de modelos de combustível não encontrado."
    call flush (6)
    stop
  end if
  ...
  ! Preencher latitude e longitude para cada ponto na grelha de combustível
  do j = 1, rows
    vlat(j) = first_lat + (j - 1)*cellsize
  end do
  do i = 1, ncols
    vlon(i) = first_lon + (i - 1)*cellsize
  end do
  ...
  print*,'vou calcular NFFL'
  call flush (6)
  ! Loop sobre a grelha de interesse (tcomb)
  do j = jfds, jfde
    do i = ifds, ifde
      grid_latitude = fxlat(i, j)
      grid_longitude = fxlong(i, j)
      ...
      ! Limite de busca para otimização
      do ja = max(1, int((grid_latitude - first_lat)/cellsize) - 1), &
               min(rows, int((grid_latitude - first_lat)/cellsize) + 1)
        do ia = max(1, int((grid_longitude - first_lon)/cellsize) - 1), &
                 min(ncols, int((grid_longitude - first_lon)/cellsize) + 1)

          ! Calcular distância aproximada
          dist_lat = abs(grid_latitude - vlat(ja))
          dist_lon = abs(grid_longitude - vlon(ia))

          if (dist_lat <= 0.02 .and. dist_lon <= 0.02) then
            lat1 = grid_latitude*con_grau
            lon1 = grid_longitude*con_grau
            lat2 = vlat(ja)*con_grau
            lon2 = vlon(ia)*con_grau

            delta_lat = lat2 - lat1
            delta_lon = lon2 - lon1
            
            a=Raio*2.0
            b=sin(delta_lat/2.0)**2.+cos(lat1)*cos(lat2)*sin(delta_lon/2.0)**2.
            c=1.0 - (sin(delta_lat/2.0)**2.+cos(lat1)*cos(lat2)*sin(delta_lon/2.0)**2.)  
            nearest_dist = a*atan2(sqrt(b),sqrt(c))

            if (nearest_dist < res_ant) then
              res_ant = nearest_dist   
              tcomb(i, j) = idclass(ia, ja)
            end if
          end if
        end do
      end do
  ...
  print*,'vou gravar dados NFFL'
  call flush (6)
  ! Gravar `tcomb` em arquivo binário, se necessário
  if (escrever == 1) then
    write(fname, "(a)") "NFFL_High_resol.bin"
    recl_size = 4*((ifde - ifds + 1)*(jfde - jfds + 1))
    open(unit=90, file=trim(fname), form="unformatted", access="direct", &
         status="replace", recl=recl_size)
    write(90, rec=1) ((tcomb(i, j), i = ifds, ifde), j = jfds, jfde)
    close(90)
  end if
...
!  end subroutine build_ZSF
  ...
  real, parameter :: cellsize = 0.000225
  real, parameter :: first_lat = 36.999861111111
  real, parameter :: first_lon = -10.000138888889
  real, parameter :: con_grau = 3.14159265358979323846/180.0
  real, parameter :: Raio = 6372.795477598*1000.0

  integer, parameter :: rows = 23134, ncols = 11853
  integer, parameter :: escrever = 1
  real, parameter :: res_ant_init = 1000.0
  real, allocatable :: topo_txt(:,:)
  real, allocatable :: vlat(:), vlon(:)
  ...
  inquire(file=trim(filename), exist=file_exists)
  if (.not. file_exists) then
    print *, "Arquivo de topografia não encontrado."
    call flush (6)
    stop
  end if
  ...
  print*,'Loop sobre a grelha de interesse ZSF'
  call flush (6)
  ! Loop sobre a grelha de interesse (ZSF)
  do j = jfds, jfde
    do i = ifds, ifde
      grid_latitude = fxlat(i, j)
      grid_longitude = fxlong(i, j)
      res_ant=res_ant_init
    !  print*,'Limite de busca para otimização' 
      ! Limite de busca para otimização
      do ja = max(1, int((grid_latitude - first_lat)/cellsize) - 1), &
               min(rows, int((grid_latitude - first_lat)/cellsize) + 1)
        do ia = max(1, int((grid_longitude - first_lon)/cellsize) - 1), &
          ...
          ! Calcule a distância simples (em graus)
          dist_lat = abs(grid_latitude - vlat(ja))
          dist_lon = abs(grid_longitude - vlon(ia))
          ...
          if (dist_lat <= 0.02 .and. dist_lon <= 0.02) then
            ...
            if (nearest_dist < res_ant) then
              res_ant = nearest_dist
              ZSF(i, j) = topo_txt(ia, ja)
            !  print*,'ZSF',i,j,ZSF(i, j)
            !  call flush (6)
            end if
          end if
        end do
  ...
  print*,'Gravar arquivo binário se necessário'
  call flush (6)
  ! Gravar arquivo binário se necessário
  if (escrever == 1) then
    write(fname, "(a)") "ZSF_High_resol.bin"
    recl_size = 4*((ifde - ifds + 1)*(jfde - jfds + 1))
    open(unit=90, file=trim(fname), form="unformatted", access="direct", &
         status="replace", recl=recl_size)
    write(90, rec=1) ((ZSF(i, j), i = ifds, ifde), j = jfds, jfde)
    close(90)
  end if
  print*,'Desalocar arrays'
  call flush (6)
  ! Desalocar arrays
  deallocate(topo_txt)
  deallocate(vlat)
