...
contains
   ...
   subroutine fuel_moisture( &
    ...
    !  call flush (6)
      ...
      !FFR call check_mesh_2dim(ifds, ifde, jfds, jfde, ifms, ifme, jfms, jfme, fmc_f, 'fuel_moisture: fmc_f')
            ...
            do i = ifds, ifde
               ...
               if (n > 0) then
                  if (k .ne. kfmc_ndwi) then
                     ...
                     !######### INTRODUZIDO POR ISILDACM
                     if(fmc_g(i, j) .ne. fmc_g(i, j)) then
                        fmc_g(i, j) = 0.0  ! Substituir NaN por zero
                     end if
                     !##########
                     write(46,*) i,j,n, fmc_g(i,j),fmc_gw(n, k),fmc_f(i, j)
                  else
                     f1 = fmc_f(i, j)
                     w1 = fmc_gl_stdev(n)
                     w1 = 1./(w1*w1)
                     ...
                     !########## 
                     write(47,*) i,j,n,fmc_g(i,j),fc,fa,f2,w1,w2,f1
                     write(31,*)i,j,n,fmc_gl_ndwi_0(n),fmc_gl_ndwi_rate(n),fndwi(i, j),fmc_gl_stdev(n)
                  end if
               end if
            end do
         end do
