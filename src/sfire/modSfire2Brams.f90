...
contains
   ...
   ! ------------------------------------------------------------------------------------------------
    ...
    subroutine get_emission_in_global_brams_grid(glat,glon,dn0)
        ...
        !write(65,*) 'Entrando em get_emission_in_global_brams_grid!!!!'
        !write(65,*) 'LFR-DBG: nfocos: ',nfocos
        ...
        do i = 1, nfocos
            ...
            write(65,*) i,x,y,count_in_grid(x,y), b_fires_lat (count_in_grid(x,y),x,y),b_fires_lon (count_in_grid(x,y),x,y),b_fires_frp (count_in_grid(x,y),x,y),b_fires_area(count_in_grid(x,y),x,y),b_fires_time(count_in_grid(x,y),x,y)
        end do
        ...
        do j = 1,nnyp(ifm)
            do i = 1,nnxp(ifm)
                ...
                write(66,*) i,j, mean_frp(i,j),mean_size(i,j), flam_frac(i,j), std_frp(i,j), std_size(i,j)
            end do
        end do
    ...
    subroutine comm_aer_data(mzp, mxp, myp, ia, iz, ja, jz, temp_sflux_t, temp_sflux_r)
        use node_mod, only: &
            mynum, &
            mchnum, &
            master_num, &
        ...
        real, intent(in) :: temp_sflux_t(nnxp(1),nnyp(1)), temp_sflux_r(nnxp(1),nnyp(1))
        ...
        real :: tflux_com(nnxp(1),nnyp(1))
        real :: rflux_com(nnxp(1),nnyp(1))
        ...
        if (mchnum == master_num) then
            ...
            tflux_com = temp_sflux_t
            rflux_com = temp_sflux_r
        end if
        ...
        !Send master frp data to all processors
        ...
        !Send master flux to all processors
        call parf_bcast(tflux_com    , int(nnxp(ifm),i8), int(nnyp(ifm),i8),master_num)
        call parf_bcast(rflux_com    , int(nnxp(ifm),i8), int(nnyp(ifm),i8),master_num)        


		write(*,*) "Comunicados: ",mynum,maxval(mean_frp_com),maxval(mean_size_com),maxval(flam_frac_com)
        ...
        !Getting local part of fluxes

        call copy_buff2D(a=tflux_com, b=pvar, &
        n2=nnxp(ifm), n3=nnyp(ifm), m2=nodemxp(mynum,ifm), m3=nodemyp(mynum,ifm), &
        i1=i1, i2=i2, j1=j1, j2=j2)
        turb_g(ifm)%sflux_t = pvar  

        call copy_buff2D(a=rflux_com, b=pvar, &
        n2=nnxp(ifm), n3=nnyp(ifm), m2=nodemxp(mynum,ifm), m3=nodemyp(mynum,ifm), &
        i1=i1, i2=i2, j1=j1, j2=j2)
        turb_g(ifm)%sflux_r = pvar  
    ...
    subroutine fill_sfire_info_and_get_emissions(ifds,ifde,jfds,jfde,frp,fire_area,latf,lonf,tburn,glat,glon,dn0)
        ...
        !write(51,fmt='("Inside| isize,jsize: ",2(I5.5,1X))') isize,jsize

        !### INTRODUZIDO POR ISILDA
        !open(55,file='fort_calor1.txt',status='unknown',form='formatted',action='write')
        !open(56,file='fort_calor2.txt',status='unknown',form='formatted',action='write')
        !open(57,file='fort_calor3.txt',status='unknown',form='formatted',action='write')
        !######
        ...
        do j = 1, jsize
            do i = 1 ,isize
                if(frp(i,j)>0.0 .and. fire_area(i,j)>0.0) then
                    !write(55,*) j,i,"frp",frp(i,j),"fire_area",fire_area(i,j)
                    ifoc = ifoc+1
                    ival(ifoc) = i
                    jval(ifoc) = j
                end if
	   ...
	   !write(56,*) "================================================================ ",ifoc
        ...
        do i = 1 ,ifoc
            sfire_info_lat (i) = latf(ival(i),jval(i))
            sfire_info_lon (i) = lonf(ival(i),jval(i))
            sfire_info_frp (i) = frp(ival(i),jval(i))
            sfire_info_area(i) = fire_area(ival(i),jval(i))
            sfire_info_time(i) = tburn(ival(i),jval(i))
            !if(sfire_info_frp (i) > 0.0) then
            ! 	write(56,fmt='("i",I4.4," - lat: ",F6.2," - lon: ",F6.2," - FRP: ",E16.6," - Area: ",E16.6," - Time: ",E16.6)') i,sfire_info_lat (i) &
            !	        ,sfire_info_lon(i), sfire_info_frp(i),sfire_info_area(i),sfire_info_time(i)
            !endif
        end do        
