...
subroutine FillNetcdfVarControlFile(oneNamelistFile, onePostGrid, oneBramsGrid)
    use netcdf, only: nf90_enddef, nf90_set_fill, nf90_nofill
    implicit none
    ...
    call DefineNetcdfDimensions(onePostGrid, oneBramsGrid)

    call DefineNetcdfCoordVars()

    call DefineNetcdfModelVars(oneNamelistFile)

    iErrNumber = nf90_enddef(ncid)
    CHECK_NF90(iErrNumber)

    call PutNetcdfCoordVars(onePostGrid, oneBramsGrid)
...
subroutine DefineNetcdfDimensions(onePostGrid, oneBramsGrid)
...
end subroutine DefineNetcdfDimensions


subroutine DefineNetcdfCoordVars()
    use mem_grid, only: iyear1, imonth1, idate1, ihour1
    use netcdf, only: nf90_def_var, nf90_put_att, nf90_float, nf90_int
    implicit none
    ...
    write(datestring, '(I4.4, "-", I2.2, "-", I2.2, " ", I2.2, ":00:00")') &
...
end subroutine DefineNetcdfCoordVars


subroutine DefineNetcdfModelVars(oneNamelistFile)
    use mem_grid, only: npatch
    use netcdf, only: nf90_def_var, nf90_put_att, nf90_float
    implicit none

    type(NamelistFile), pointer :: oneNamelistFile

    type(PostVarType) :: one_post_variable
    integer :: iErrNumber, i, ivp, all_post_idx, cnt
    ...
    do ivp = 1, oneNamelistFile%nvp
        ...
        if (len(trim(one_post_variable%fieldName)) .eq. 0) then
            print *, "**(OnePostField)** Post field " // vName // " does not exist in list of variables."
    ...
    ! Write model variable attributes
    do i = 1, cnt
        ...
        iErrNumber = nf90_put_att(ncid, VarDimId(i), "units", trim(netCdfFieldUnits(i)))
        CHECK_NF90(iErrNumber)

        iErrNumber = nf90_put_att(ncid, VarDimId(i), "long_name", trim(netCdfFieldDescription(i)))
        CHECK_NF90(iErrNumber)
    end do

end subroutine DefineNetcdfModelVars


subroutine PutNetcdfCoordVars(onePostGrid, oneBramsGrid)
    use mem_grid, only: time
...
end subroutine PutNetcdfCoordVars
