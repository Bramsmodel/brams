...
module ModCupGrellCattDeep
  ...
  use mem_scratch3_grell, only: &
       aa0, aa1, aad, bu, cap_max, cap_max_increment, cd, cdd, cwf, dby, &
       dbyd, dbydo, dbyo, dellah, dellaq, dellaq_ens, dellaqc, dellaqc_ens, &
       dellat, dellat_ens, eddt, edt, edt_ens, edtc, edto, edtx, gamma_cup, &
       gammao_cup, hc, hcd, hcdo, hco, he, he_cup, heo, heo_cup, hes, &
       hes_cup, heso, heso_cup, hkb, hkbo, ierr, ierr2, ierr3, jmin, k22, &
       k22x, kbcon, kbconx, kbmax, kstabi, kstabm, ktop, kzdown, kzi, &
       mbdt_ens, outt_ens, p_cup, po_cup, pr_ens, predb, pw, pwav, pwavo, &
       pwd, pwdf, pwdo, pwev, pwevo, pwf, pwo, pwo_ens, q_cup, qc, qcd, &
       qcdo, qco, qes, qes_cup, qeso, qeso_cup, qo_cup, qrc, qrcd, qrcdo, &
       qrco, scr1, sdp, t_cup, tn_cup, vshear, vws, xaa0, xaa0_ens, xdby, &
       xf_ens, xfac1, xfac_for_dn, xff_ens3, xhc, xhcd, xhe, xhe_cup, xhes, &
       xhes_cup, xhkb, xk, xmass, xmb, xpw, xpwav, xpwd, xpwev, xq, xq_cup, &
       xqc, xqcd, xqes, xqes_cup, xqrc, xqrcd, xt_cup, xt_grell, xz, &
       xz_cup, xzd, xzu, z, z_cup, zd, zdo, zo, zo_cup, zu, zuo

  use kbcon_ecmwf, only: &
       trigg_ecmwf
  ...
  use mem_scratch2_grell, only : &
       direction, omeg, outq, outqc, outt, p, po, psur, q, qo, rcpg, t, &
       ter11, tkeg, tn, us_grell, vs_grell, aa0_scratch2=>aa0, glatg, glong, iact_gr, &
       iact_old_gr, kdet, kdt, massfln, massflx, mconv, pmean, pret, umean, &
       vmean, xland
...
contains
  ...
  subroutine cuparth_catt(ccatt,  &
       mynum, mgmxp, mgmyp, mgmzp, m1, m2, m3, ia, iz, ja, jz, i0, &
       j0, maxiens, iens, ngrid, ngrids_cp, dtime, time, ua, va, wa, &
       theta, thetail, pp, pi0, dn0, rv, tke, tkmin, rcp, rrp, rpp, &
       rsp, rap, rgp, rhp, topt, rtgt, tht, rtt, pt, outtem, outrt, &
       outcl, precip, ierr4d, jmin4d, kdet4d, k224d, kbcon4d, ktop4d, &
       kpbl4d, kstabi4d, kstabm4d, xmb4d, edt4d, zcup5d, pcup5d, &
       enup5d, endn5d, deup5d, dedn5d, zup5d, zdn5d, prup5d, clwup5d, &
       tup5d, upmf, dnmf, xierr, xktop, xkbcon, xk22, xjmin, xkdt, &
       xiact_p, xiact_c, confrq, frqanl, deltaxy2, patch_area, npat, &
       level, glat, glon, sflux_r, sflux_t, trigg, autoconv)
    ...
    real, intent(in) :: time
    real, dimension(:,:,:), intent(in) :: ua, va, wa, theta, pp, pi0, dn0,   &
         rv, tht, rtt, &
         tke,rcp,rrp,rpp,rsp,rap,rgp,rhp,thetail
    real, intent(in) :: pt(m1,m2,m3)
    real, dimension(:,:,:), intent(in)  :: patch_area
    real, intent(in) :: tkmin, confrq, frqanl, deltaxy2
    real, dimension(:,:,:), intent(inout) :: outtem, outrt,outcl
    real, dimension(:,:), intent(in)    :: topt,glat,glon,sflux_r,sflux_t
    real, dimension(:,:), intent(inout) :: PRECIP
    real, dimension(:,:), intent(in)    :: rtgt
    ...
    do j=j1,j2
       ...
       call CUP_enss_catt(ccatt,ngrid,mynum,m1,m2,m3,i0,j0,ipr,jpr,             &
            ...
            aa0_scratch2, t, q, tn, qo, po, pret, p, outt, outq, outqc,            &
...
505             continue
  ...
  !--------------------------------------------------------------------
end module ModCupGrellCattDeep
