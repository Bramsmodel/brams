...
contains
  ...
  subroutine ctrlvols (n1,n2,n3,nstbot,dzt,xm,ym,zm,platn,plonn,  &
    ...
    real, intent(out) :: aru(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: arv(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: arw(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: volt(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: volu(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: volv(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: volw(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: lpu(:,:) ! (n2,n3)
    real, intent(inout) :: lpv(:,:) ! (n2,n3)
    real, intent(inout) :: lpw(:,:) ! (n2,n3)
    real, intent(in) :: dxu(:,:) ! (n2,n3) **(JP)** unused
    real, intent(in) :: dxv(:,:) ! (n2,n3)
    real, intent(in) :: dxt(:,:) ! (n2,n3)
    real, intent(in) :: dyu(:,:) ! (n2,n3)
    real, intent(in) :: dyv(:,:) ! (n2,n3) **(JP)** unused
    real, intent(in) :: dyt(:,:) ! (n2,n3)
    real, intent(inout) :: topma(:,:) ! (n2,n3)
...
316    format('arv3 - second topm adjustment performed ',2i4,2f8.1)
    ...
    call adap_bldg(n1,n2,n3,xm,ym,zm,platn,plonn  &
         ,aru,arv,arw,topma,ifm, nzg, npatch)
  ...
  subroutine adap_bldg(n1,n2,n3,xm,ym,zm,platn,plonn,aru,arv,arw,topma  &
    ...
    real, intent(in) :: platn
    real, intent(in) :: plonn
    real, intent(inout) :: aru(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: arv(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: arw(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: topma(:,:) ! (n2,n3)
    integer, intent(in) :: ifm
    integer, intent(in) :: nzg
    integer, intent(in) :: npatch
...
!!$  end subroutine setgv
  ...
  subroutine advected(m1,m2,m3,uc,vc,wc,ucb,vcb,wcb,lpu,lpv,lpw,i,j,jdim)
    ...
    integer, intent(in) :: lpu(:,:) ! (m2,m3)
    integer, intent(in) :: lpv(:,:) ! (m2,m3)
