...
contains
  ...
  subroutine inithh(oneBasicFields, oneMicVars)
    ! +--------------------------------------------------------+
...
1      format('    Input sounding needs to go higher ! !', /,  &
  ...
  subroutine flds3d(n1,n2,n3,uc,vc,pi0,theta,thp,rtp,pc,rv  &
       ,topt,topu,topv,rtgt,rtgu,rtgv, oneMicVars)
    type(MicControl), pointer, intent(in) :: oneMicVars
    integer :: n1,n2,n3
    ...
    do j=1,nyp
       do i=1,nxp
          ...
          if(oneMicVars%level.eq.1)then
             do k=1,nzp
                rv(k,i,j)=rtp(k,i,j)
             enddo
          endif

          if(oneMicVars%level.ge.2)then
             do k=1,nzp
