  ...
  subroutine diffuse(oneScalarTab, oneScalarTabSize, oneBasicFields, &
    ...
    if (if_adap==0) then
       ...
       call strain_adap(mzp,mxp,myp,ia,iz,ja,jz                  &
            ...
            ,grid_g(ngrid)%lpw (:,:)   ,oneBasicFields%up (:,:,:)  &
    ...
    if (idiffk(ngrid) == 7) then
       ...
       call nakanishi(mzp, mxp, myp, npatch, ia, iz, ja, jz, jdim                           &
            ...
            ,oneBasicFields%theta     ,oneBasicFields%rv        ,oneBasicFields%rtp     &
                                !
    ...
    if (CCATT==1 .and. chemistry >= 0) then
       ...
       call klbnd(mzp,mxp,myp,ibcon,jdim,scr3(:) &
            ,oneBasicFields%dn0(:,:,:),grid_g(ngrid)%lpw(:,:))
    endif
    ...
    if (if_adap==0) then
       !call dumpVarAllLatLonk(oneBasicFields%up, 'tUP'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       !call dumpVarAllLatLonk(oneBasicFields%Vp, 'tVP'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       !call dumpVarAllLatLonk(oneBasicFields%Wp, 'tWP'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       !call dumpVarAllLatLonk(oneBasicFields%dn0, 'DN0'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       !call dumpVarAllLatLonk(oneBasicFields%dn0v, 'DN0V'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       !call dumpVarAllLatLonk(oneBasicFields%dn0u, 'DN0U'  ,415,0,0,1,mxp,1,myp,1,mzp,0.0,0.0) !ok
       ...
       if (if_adap==0) then
          ...
          if (nnqparm(ngrid)>=2) then
             ...
             if (oneScalarTab(n)%name=='THP' .or. oneScalarTab(n)%name=='THC')     &
                  ...
                  scratch%vt3df, oneScalarTab(n)%a_var_p_1D, cuforc_sh_g(ngrid)%lsfth, n)

             if (oneScalarTab(n)%name=='RTP')                                          &
                  call PBLforcing(ngrid, mzp, mxp, myp, ia, iz, ja, jz, &
                  scratch%vt3df, oneScalarTab(n)%a_var_p_1D, cuforc_sh_g(ngrid)%lsfrt, n)

          endif
       else

          call diffsclr_adap(mzp,mxp,myp,ia,iz,ja,jz,jdim,n,ksf      &
               ,grid_g(ngrid)%lpw(:,:)     ,oneScalarTab(n)%a_var_p_1D&
               ,oneScalarTab(n)%a_var_t_1D,scratch%vt3da     (:)      &
               ,scratch%vt3dc      (:)     ,scratch%vt3df     (:)      &
               ,scratch%vt3dg      (:)     ,scratch%vt3dj     (:)      &
               ,scratch%vt3dk      (:)     ,scratch%vt3dl     (:)      &
               ,scratch%vt3dm      (:)     ,scratch%vt3do     (:)      &
               ,scratch%vt2da      (:)     ,scratch%vt2db     (:)      &
               ,oneBasicFields%dn0 (:,:,:) ,vkh_p                      &
