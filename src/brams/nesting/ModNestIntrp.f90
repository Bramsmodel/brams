...
module ModNestIntrp
  ...
  use mem_grid, only: &
       dztn, ei1, ei2, ei3, ei4, ei5, ei6, ei7, ej1, ej2, ej3, ej4, ej5, &
       ej6, ej7, ek1, ek2, ek3, ek4, ek5, ek6, ek7, fbcf, grid_g, ipm, &
...
contains
  ...
  subroutine fmrefs3d(ifm, oneBasicFields, otherBasicFields)
    integer, intent(in) :: ifm
    type(BasicFields), pointer, intent(in) :: oneBasicFields  ! at current grid
    type(BasicFields), pointer, intent(in) :: otherBasicFields! at some other grid

    integer :: icm,l,i,j,k,ierr
    real :: c1,c2
    ...
    do j = 1,nnyp(ifm)
       do i = 1,nnxp(ifm)
          do k = 1,nnzp(ifm)
             otherBasicFields%pi0(k,i,j) = c2  &
                  * (otherBasicFields%dn0(k,i,j) * otherBasicFields%th0(k,i,j)) ** c1
          enddo
       enddo
    enddo
    ...
    call rtgintrp(nnzp(ifm),nnxp(ifm),nnyp(ifm)  &
         ,otherBasicFields%th0  &
         ,scr1,grid_g(ifm)%topt,ifm,'t')
    call rtgintrp(nnzp(ifm),nnxp(ifm),nnyp(ifm)  &
         ,otherBasicFields%pi0  &
         ,scr1,grid_g(ifm)%topt,ifm,'t')
    ...
    call fill_dn0uv(nnzp(ifm),nnxp(ifm),nnyp(ifm)  &
         ,otherBasicFields%dn0 &
         ,otherBasicFields%dn0u,otherBasicFields%dn0v)
...
!            ek6, ek7)
  ...
  subroutine nstbtnd(m1,m2,m3,ia,iz,ja,jz,ibcon  &
    ...
    real, intent(in) :: bx(m1,m3,2)
    real, intent(in) :: by(m1,m2,2)
    real, intent(in) :: bz(m2,m3,2)
    character(len=*), intent(in) :: vnam
    real, intent(in) :: tymeinv
    integer, intent(in) :: nstbot
    integer, intent(in) :: nsttop
    integer, intent(in) :: jdim
...
50     continue
  ...
  subroutine fmint4(var1,var2,dn0xc,dn0xf,vt2da,ifm,icm,vpnt,idwt)
    real, intent(in) :: var1(*)
    real, intent(inout) :: var2(*)
    real, intent(in) :: dn0xc(*)
    real, intent(in) :: dn0xf(*)
    real, intent(in) :: vt2da(*)
    integer, intent(in) :: ifm
    integer, intent(in) :: icm
    character(len=*), intent(in) :: vpnt
    integer, intent(in) :: idwt
