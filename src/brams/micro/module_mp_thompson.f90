...
CONTAINS
  ...
  SUBROUTINE thompson_init(hgt, &!, orho, nwfa2d, nwfa, nifa,          &
    ...
    if(mchnum==master_num) then
       call wrf_message(" ")
       call wrf_message("========================================================================")
       call wrf_message("initializing thompson microphysics")
    end if
    ...
    if(mchnum==master_num) then
       call wrf_message("done initializing thompson microphysics")
       call wrf_message("========================================================================")
       call wrf_message(" ")
    end if
...
151    continue
  ...
  subroutine qr_acr_qg
    ...
    LOGICAL, PARAMETER :: write_thompson_tables = .true.
    ...
    IF ( wrf_dm_on_monitor) THEN
       ...
       if(mchnum==master_num) then 
          ...
          IF ( lexist ) THEN
             write (22,fmt='(A)') "qr_acr_qg at ThompMP: read ./tables/micro_GT/qr_acr_qgV3.dat instead of computing"
             call wrf_message("read ./tables/micro_GT/qr_acr_qgV3.dat instead of computing")
          ELSE
             write (22,fmt='(A)') "qr_acr_qg at ThompMP: file ./tables/micro_GT/qr_acr_qgV3.dat not found"
             call wrf_message("file ./tables/micro_GT/qr_acr_qgV3.dat not found")
          endif
          close(unit=22)
       END IF
...
1234      CONTINUE
    ...
    IF ( good .EQ. 1 ) THEN
       ...
       !#endif
    ELSE
       CALL wrf_message("computing qr_acr_qgV3")
       do n2 = 1, nbr
  ...
  subroutine qr_acr_qs
    ...
    IF ( wrf_dm_on_monitor ) THEN
       ...
       if(mchnum==master_num) then 
          open(unit=22,file='brams.log',position='append',action='write')
          IF ( lexist ) THEN
             write (unit=22,fmt='(A)') "qr_acr_qs at ThompMP: read ./tables/micro_GT/qr_acr_qsV2.dat instead of computing"
             call wrf_message("read ./tables/micro_GT/qr_acr_qsV2.dat instead of computing")
          else
             write (unit=22,fmt='(A)') "qr_acr_qs at ThompMP: file ./tables/micro_GT/qr_acr_qsV2.dat not found"
             call wrf_message("file ./tables/micro_GT/qr_acr_qsV2.dat not found")
          endif
          close(unit=22)
       end IF
...
1234      CONTINUE
    ...
    IF ( good .EQ. 1 ) THEN
       ...
       !#endif
    ELSE
       CALL wrf_message("computing qr_acr_qsV2")
       do n2 = 1, nbr
          !        vr(n2) = av_r*Dr(n2)**bv_r * DEXP(-fv_r*Dr(n2))
          vr(n2) = -0.1021 + 4.932E3*Dr(n2) - 0.9551E6*Dr(n2)*Dr(n2)     &
               + 0.07934E9*Dr(n2)*Dr(n2)*Dr(n2)                          &
  ...
  subroutine freezeH2O
    ...
    IF ( wrf_dm_on_monitor ) THEN
       ...
       if(mchnum==master_num) then 
          ...
          open(unit=22,file='brams.log',position='append',action='write')
          IF ( lexist ) THEN
             write (unit=22,fmt='(A)') "freezeH2O at ThompMP: read ./tables/micro_GT/freezeH2O.dat instead of computing"
             call wrf_message("read ./tables/micro_GT/freezeH2O.dat instead of computing")
          else
             write (unit=22,fmt='(A)') "freezeH2O at ThompMP: file ./tables/micro_GT/freezeH2O.dat not found"
             call wrf_message("file ./tables/micro_GT/freezeH2O.dat not found")
          end IF
          close(unit=22)
       end IF
...
1234      CONTINUE
    ...
    IF ( good .EQ. 1 ) THEN
       ...
       !#endif
    ELSE
       CALL wrf_message("computing freezeH2O")
