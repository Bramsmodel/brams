...
contains
  ...
  subroutine SstReadStoreOwnChunk(runflag, ifm, ierr, oneControlVars)
    ...
    elseif (runflag==3) then   ! Runtime file increment
       ...
       if (time>=ssttime2(ifm)) then
          ...
          call SstUpdate(1, isstflf(ifm), oneControlVars)
...
!!$  nsstfiles(ngrids+1:)=0
  ...
  subroutine SstUpdate(iswap, nfile, oneControlVars)
