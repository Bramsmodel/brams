...
contains
  ...
  subroutine NdviReadStoreOwnChunk(runflag, ifm, ierr, oneControlVars)
    ! Arguments:
    ...
    if (runflag==1 .or. runflag==2) then   ! Initialization(1) or file check(2)
       ...
       do ng=1,ngrids
          ...
          call NdviUpdate(0, indviflp(ng), oneControlVars)
          if (iupdndvi==1) then
             ! Read future time ndvi field if updating
             call NdviUpdate(1, indviflf(ng), oneControlVars)
    ...
    elseif (runflag==3) then   ! Runtime file increment
       ...
       if (time>=ndvitime2(ifm) ) then
          ...
          call NdviUpdate(1, indviflf(ifm), oneControlVars)
...
!!$  nndvifiles(ngrids+1:)=0
  ...
  subroutine NdviUpdate(iswap, nfile, oneControlVars)
