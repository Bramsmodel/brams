...
module ModLeaf3Init
  ...
  use ModLeafComs, only: &
       ...
       dead_frac, rcmin, glai_max, root, kroot, timefac_sst
...
contains
  ...
  subroutine sfcinit_file(n2,n3,mzg,npat,ifm,patch_area,leaf_class,soil_text)
    ...
    integer, intent(in) :: ifm !**(JP) unused
    real, intent(out) :: patch_area(n2,n3,npat)
    real, intent(out) :: leaf_class(n2,n3,npat)
    real, intent(out) :: soil_text(mzg,n2,n3,npat)
  ...
  subroutine sfcinit_nofile(n1,n2,n3,mzg,mzs,npat,ifm  &
    ...
    real, dimension(n2,n3)    :: rvv,prsv,piv,glat,glon,zot  &
...
!!!    10, 2,17, 1, 0,12,13,14,18, 4,  & ! 10 (use for future Olson data)
  ...
  subroutine datp_datsoil(datp,datsoil)
    ...
    integer, parameter :: catb(0:133)=(/6  &
         , 6, 4, 4, 7, 7, 8, 6, 4, 4, 4  &
         , 7, 4, 4, 4, 8, 4, 8, 4, 4, 8  &
         , 4, 2, 4, 4, 4, 4, 6, 8, 8, 8  &
         , 4, 8, 8, 2, 6, 4, 7, 4, 4, 3  &
