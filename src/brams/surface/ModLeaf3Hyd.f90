...
contains
  ...
  subroutine hydro(m2,m3,mzg,mzs,np,&
    ...
    real, intent(inout) :: soil_energy(:,:,:,:)
    real, intent(in) :: soil_text(:,:,:,:)
    real, intent(inout) :: sfcwater_mass(:,:,:,:)
    real, intent(inout) :: sfcwater_energy(:,:,:,:)
    real, intent(inout) :: patch_area(:,:,:)
    real, intent(inout) :: patch_wetind(:,:,:)
    
    integer :: ngrp,ng,l,i,j,ip
...
12        format(15x,f5.3,20f5.1)
  ...
  subroutine hydrol(m2,m3,mzg,mzs,np,maxpatch,ngrps          &
    ...
    integer, intent(in) :: lpg(:)
    real, intent(in) :: slmsts(:)
    real, intent(out) :: zi(:)
    real, intent(out) :: fa(:)
    real, intent(out) :: wi(:)
    real, intent(in) :: finv(:)
    real, intent(in) :: rhow !**(JP)** unused
    real, intent(in) :: rhowi !**(JP)** unused
    real, intent(in) :: dtlt
    real, intent(in) :: slcons0(:)
    real, intent(inout) :: slcpd(:)
    real, intent(out) :: wateradd(:)
    real, intent(in) :: time !**(JP)** unused

    integer :: ngd,i,j,k,lp,l,ip,nsoil,ibotpatch
