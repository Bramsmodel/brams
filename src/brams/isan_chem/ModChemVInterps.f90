...
contains
  ...
  subroutine obs_isen (m1,m2,tsnd,psnd,zsnd,rsnd,usndz,vsndz  &
    ...
    real, intent(in) :: tsnd(:,:) ! (m1,m2)
    real, intent(in) :: psnd(:,:) ! (m1,m2)
    real, intent(in) :: zsnd(:,:) ! (m1,m2)
    real, intent(in) :: rsnd(:,:) ! (m1,m2)
    real, intent(in) :: usndz(:,:) ! (m1,m2)
    real, intent(in) :: vsndz(:,:) ! (m1,m2)
    real, intent(in) :: zsndz(:,:) ! (m1,m2)
    real, intent(in) :: stlt(:) ! (m1) **(JP)** unused
    real, intent(in) :: stln(:) ! (m1) **(JP)** unused
    real, intent(in) :: topsnd(:) ! (m1) **(JP)** unused
    integer, intent(in) :: lpst(:) ! (m1)
    integer, intent(in) :: lzst(:) ! (m1)
    real, intent(inout) :: obsu(:,:) ! (no,*)
    real, intent(inout) :: obsv(:,:) ! (no,*)
    real, intent(inout) :: obsp(:,:) ! (no,*)
    real, intent(inout) :: obss(:,:) ! (no,*)
    real, intent(inout) :: obsr(:,:) ! (no,*)
    integer, intent(in) :: no ! **(JP)** unused
    character(len=8), intent(in) :: chstid(:)
...
600    continue
  ...
  subroutine obs_sigz (m1,m2,tsnd,psnd,zsnd,rsnd,usndz,vsndz  &
    ...
    integer, intent(in) :: lpst(:) ! (m1)
    integer, intent(in) :: lzst(:) ! (m1)
    real, intent(in) :: sndtopg(:) ! (m1)
    real, intent(inout) :: obsu(:,:) ! (no,*)
    real, intent(inout) :: obsv(:,:) ! (no,*)
    real, intent(inout) :: obsp(:,:) ! (no,*)
    real, intent(inout) :: obst(:,:) ! (no,*)
    real, intent(inout) :: obsr(:,:) ! (no,*)
...
600    continue
  ...
  subroutine vterpp_i (np1,np2,np3,npi3,un,vn,tn,zn,rn,ui2,vi2,pi2,si2,ri2)
    ...
    integer, intent(in) :: np3
    integer, intent(in) :: npi3
    real, intent(in) :: un(:,:,:) ! (np1,np2,np3)
    real, intent(in) :: vn(:,:,:) ! (np1,np2,np3)
    real, intent(in) :: tn(:,:,:) ! (np1,np2,np3)
    real, intent(in) :: zn(:,:,:) ! (np1,np2,np3)
    real, intent(in) :: rn(:,:,:) ! (np1,np2,np3)
    real, intent(inout) :: ui2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: vi2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: pi2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: si2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: ri2(:,:,:) ! (np1,np2,npi3)
...
4500      continue
  ...
  subroutine vterpp_s (np1,np2,np3,npi3,un,vn,tn,zn,rn  &
    ...
    real, intent(inout) :: vi2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: pi2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: ti2(:,:,:) ! (np1,np2,npi3)
    real, intent(inout) :: ri2(:,:,:) ! (np1,np2,npi3)
    real, intent(in) :: topt(:,:) ! (np1,np2)
    real, intent(in) :: rtgt(:,:) ! (np1,np2)
...
4500      continue
  ...
  subroutine psfill(nz,vvv,aaa,np1,np2,i,j)
    ...
    integer, intent(in) :: i
    integer, intent(in) :: j
    real, intent(in) :: vvv(:) ! (nz)
    real, intent(inout) :: aaa(:,:,:) ! (np1,np2,nz)
