...
module ModChemAsti2
  ...
  use isan_coms, only: &
       idate, ihour, imonth, inrawi, insrfce, iobswin, iproc_dates, iyear, &
       levpr, maxlev, maxsfc, maxsname, maxsta, natime, notid, notsta, &
       nprz, nssfc, nsta, sf_chstid, sf_date, sf_lat, sf_lon, sf_p, sf_r, &
       sf_s, sf_t, sf_top, sf_u, sf_v, stasep, up_chstid, up_lat, up_lon, &
       up_lp, up_lz, up_p, up_r, up_t, up_top, up_uz, up_vz, up_z, up_zz
...
contains
  ...
  subroutine input_rawi (olat1,olat2,olon1,olon2)
    real, intent(in) :: olat1 !**(JP)** unused
    real, intent(in) :: olat2 !**(JP)** unused
    real, intent(in) :: olon1 !**(JP)** unused
    real, intent(in) :: olon2 !**(JP)** unused
...
110 continue
  ...
  subroutine sndproc (lp,lz,xlat,xlon,elev,idsta,tp,zp,pp,rp,dz,fz,zz)
    ...
    integer, intent(inout) :: lz
    real, intent(in) :: xlat
    real, intent(in) :: xlon
    real, intent(in) :: elev
    character(len=*), intent(in) :: idsta
    real, intent(inout) :: tp(:)
    real, intent(inout) :: zp(:)
    real, intent(inout) :: pp(:)
    real, intent(inout) :: rp(:)
...
1004 format(//'  Height levels',//,T5,'Z(m)',T15,'U(m/s)',  &
  ...
  subroutine input_sfc (olat1,olat2,olon1,olon2)
    real, intent(inout) :: olat1 !**(JP)** unused
    real, intent(inout) :: olat2 !**(JP)** unused
    real, intent(inout) :: olon1 !**(JP)** unused
    real, intent(inout) :: olon2 !**(JP)** unused
