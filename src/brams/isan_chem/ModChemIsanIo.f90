...
!###########################################################################
module ModChemIsanIo

  use isan_coms, only: &
       gobrad, gobsep, idate, idd, igridfl, ihh, ihour, imm, imonth, iyear, &
       iyy, levth, nisn, nsigz, pi_p, pi_r, pi_s, pi_scra, pi_scrb, pi_u, &
...
201    format(//,a,//  &
  ...
  subroutine vmissr (af,n,fm,fx)
    ...
    do j=1,size(af,1)
       do i=1,size(af,2)
          if(af(i,j)<=fx) af(i,j)=fm
       end do
    end do
  end subroutine vmissr

end module ModChemIsanIo
