...
module ModChemAObj
  ...
  use isan_coms, only: &
       gobrad, gobsep, gridwt, igridfl, maxsfc, nisn, nsigz, nssfc, nsta, &
       pi_p, pi_r, pi_s, pi_u, pi_v, ps_p, ps_r, ps_t, ps_u, ps_v, respon, &
       rs_p, rs_r, rs_s, rs_sfp, rs_sft, rs_slp, rs_snow, rs_sst, rs_t, &
       rs_top, rs_u, rs_v, sf_lat, sf_lon, sf_p, sf_r, sf_s, sf_t, sf_top, &
       sf_ur, sf_vr, swvlnth, up_lat, up_lon, upi_p, upi_r, upi_s, upi_u, &
       upi_v, ups_p, ups_r, ups_t, ups_u, ups_v, wvlnth
...
contains
  ...
  subroutine obj_anal (ctype,ng,nxp,nyp,g_lat,g_lon  &
    ...
    real, intent(inout) :: g_lat(*)
    real, intent(inout) :: g_lon(*)
    real, intent(inout) :: polelat
    real, intent(inout) :: polelon
    real, intent(inout) :: swx
    real, intent(inout) :: swy
    real, intent(inout) :: delx
    real, intent(inout) :: dely
...
!    real, allocatable :: pscr(:,:)
  ...
  subroutine obanl (npx,npy,npz,nvar,dat_p_i    ,p_lat   ,p_lon  &
    ...
    real, intent(in) :: dat_p_i(*)
    real, intent(in) :: p_lat(*)
    real, intent(in) :: p_lon(*)
    integer, intent(in) :: max_obs
    integer, intent(in) :: n_obs
    real, intent(in) :: dat_obs(*)
    real, intent(in) :: dlat_obs(*)
    real, intent(in) :: dlon_obs(*)
    integer, intent(in) :: nxp
    integer, intent(in) :: nyp
    real, intent(inout) :: dat_i_i(*)
    real, intent(in) :: g_lat(*)
    real, intent(in) :: g_lon(*)
    integer, intent(in) :: igrdflg
    real, intent(in) :: wvlnth
    real, intent(in) :: respon
    real, intent(in) :: gobsep
    real, intent(in) :: gobrad
    real, intent(in) :: gridwt
    real, intent(in) :: polat
...
90  format(/' Objective analysis---','  No. obs -',I5  &
  ...
  subroutine prebarn (nprx,npry,nisn,nvar  &
    ...
    integer, intent(in) :: nisn
    integer, intent(in) :: nvar
    real, intent(in) :: grid(nprx,npry,nisn,*)
    real, intent(in) :: glat(nprx,npry)
    real, intent(in) :: glon(nprx,npry)
    integer, intent(in) :: ist
    real, intent(in) :: ssdat(ist,nisn,*)
    real, intent(in) :: sslt(*)
    real, intent(in) :: ssln(*)
    integer, intent(in) :: nnsta
    integer, intent(in) :: igns
    real, intent(inout) :: gsdat(igns,nisn,*)
    real, intent(out) :: gsx(*)
    real, intent(out) :: gsy(*)
    real, intent(out) :: gswt(*)
    integer, intent(out) :: ngs
    integer, intent(in) :: igridfl
    real, intent(in) :: gobsep
    real, intent(in) :: gobrad
    real, intent(in) :: gridwt
    real, intent(out) :: scra(*)
    real, intent(in) :: polat
    real, intent(in) :: polon
...
80  continue
  ...
  subroutine bn_pass(grid,nxx,nyy,nlev,nvar,glat,glon  &
    ...
    real, intent(inout) :: grid(nxx,nyy,nlev,nvar)
    integer, intent(in) :: nxx
    integer, intent(in) :: nyy
    integer, intent(in) :: nlev
    integer, intent(in) :: nvar
    real, intent(in) :: glat(nxx,nyy)
    real, intent(in) :: glon(nxx,nyy)
    integer, intent(in) :: ist
    real, intent(inout) :: sdat(ist,nlev,nvar,*)
    real, intent(in) :: sxxx(*)
    real, intent(in) :: syyy(*)
    real, intent(in) :: swt(*)
    integer, intent(in) :: nsta
    real, intent(inout) :: scra(ist,*)
    real, intent(in) :: x4k
    real, intent(in) :: polat
    real, intent(in) :: polon
    integer, intent(in) :: npass
    integer, intent(in) :: iqflag
    real, intent(in) :: qual(nxx,nyy)
  ...
  subroutine stainterp(grid,nxx,nyy,nlev,nvar  &
    ...
    integer, intent(in) :: nvar
    integer, intent(in) :: ist
    real, intent(inout) :: sdat(ist,nlev,nvar,2)
    real, intent(in) :: sxxx(*)
    real, intent(in) :: syyy(*)
    integer, intent(in) :: nsta
    real, intent(in) :: polat !**(JP)** unused
    real, intent(in) :: polon !**(JP)** unused
    real, intent(in) :: swx
    real, intent(in) :: swy
    real, intent(in) :: delx
    real, intent(in) :: dely
