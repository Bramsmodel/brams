!############################# Change Log ##################################
! 5.0.0
!
!###########################################################################
!  Copyright (C)  1990, 1995, 1999, 2000, 2003 - All Rights Reserved
!  Regional Atmospheric Modeling System - RAMS
!###########################################################################
module ModChemRefState

  use ModNestFillDens, only: &
       fillscr, &
       fillvar

  use rconstants, only: &
       g, cp, cpor, p00, rgas
  
  use ccatt_start, only: &
       ccatt
  
  implicit none
  
  private
  
  public :: nest_interpolated_topo
  public :: fmrefs1d_isan
  public :: fmrefs3d_isan
  public :: fmdn0_isan
  public :: varfile_refstate
  
contains


    
  subroutine nest_interpolated_topo(nxc,nyc,nxf,nyf,maxx,maxy,ifm  &
       ,topo_c,topo_i,scr1,scr2)
    integer, intent(in) :: nxc
    integer, intent(in) :: nyc
    integer, intent(in) :: nxf
    integer, intent(in) :: nyf
    integer, intent(in) :: maxx
    integer, intent(in) :: maxy
    integer, intent(in) :: ifm
    real, intent(inout) :: topo_c(:,:)
    real, intent(inout) :: topo_i(:)
    real, intent(inout) :: scr1(:)
    real, intent(in) :: scr2(:)
    

    !    Fill topo_i with interpolated topography from coarser grid

    call fillscr(1,maxx,maxy,1,nxc,nyc,1,1,scr1,topo_c)
    call fillvar(1,maxx,maxy,1,nxf,nyf,1,1,scr2,topo_i)
  end subroutine nest_interpolated_topo

  

  subroutine fmrefs1d_isan(ifm,icm,n0,n1  &
       ,piref,thref,dnref,rtref)
    integer, intent(in) :: ifm
    integer, intent(in) :: icm
    integer, intent(in) :: n1
    integer, intent(in) :: n0
    real, intent(inout) :: piref(:,:)
    real, intent(inout) :: thref(:,:)
    real, intent(in) :: dnref(:,:)
    real, intent(inout) :: rtref(:,:)
    

    integer :: k
    real :: c1,c2
    real :: vctr1(n1)
    real :: vctr2(n1)
    real :: vctr3(n1)
    real :: vctr4(n1)
    integer :: ierr
    character(len=8) :: str(10)
    character(len=*), parameter :: h="**(fmrefs1d_isan)**"

    !     Interpolate the fine mesh 1-d reference state variables.

    c1 = rgas / (cp - rgas)
    c2 = cp * (rgas / p00) ** c1
    if (icm >= 1) then
       do k = 1,n1
          vctr1(k) = thref(k,icm) * dnref(k,icm)
          vctr2(k) = rtref(k,icm) * dnref(k,icm)
       enddo

!!$  call eintp(dnref(1,icm),dnref(1,ifm),n1,1,1,n1  &
!!$     ,1,1,ifm,1,'t',0,0)
!!$  call eintp(vctr1,vctr3,n1,1,1,n1,1,1,ifm,1,'t',0,0)
!!$  call eintp(vctr2,vctr4,n1,1,1,n1,1,1,ifm,1,'t',0,0)

       do k = 1,n1
          thref(k,ifm) = vctr3(k) / dnref(k,ifm)
          rtref(k,ifm) = vctr4(k) / dnref(k,ifm)
          piref(k,ifm) = c2 * (dnref(k,ifm) * thref(k,ifm)) ** c1
       enddo
    endif
  end subroutine fmrefs1d_isan

  !     *****************************************************************

  subroutine fmrefs3d_isan (ifm,icm,n1f,n2f,n3f,n1c,n2c,n3c  &
       ,maxiz,maxix,maxiy,nbot,ntop,jd  &
       ,scr1,scr2,vt2da,toptf,toptc,dn0c,dn0f,th0c,th0f  &
       ,pi0f,dn0uf,dn0vf,zt,ztop)
    integer, intent(in) :: ifm
    integer, intent(in) :: icm
    integer, intent(in) :: n1f
    integer, intent(in) :: n2f
    integer, intent(in) :: n3f
    integer, intent(in) :: n1c
    integer, intent(in) :: n2c
    integer, intent(in) :: n3c
    integer, intent(in) :: maxiz
    integer, intent(in) :: maxix
    integer, intent(in) :: maxiy
    integer, intent(in) :: nbot
    integer, intent(in) :: ntop
    integer, intent(in) :: jd
    real, intent(inout) :: scr1(:)
    real, intent(in) :: scr2(:)
    real, intent(in) :: vt2da(:) !**(JP)** unused
    real, intent(inout) :: toptf(:,:)  ! (n2f,n3f)
    real, intent(in) :: toptc(:,:) ! (n2c,n3c)
    real, intent(in) :: dn0c(:,:,:) ! (n1c,n2c,n3c) **(JP)** unused
    real, intent(in) :: dn0f(:,:,:) ! (n1f,n2f,n3f)
    real, intent(in) :: th0c(:,:,:) ! (n1c,n2c,n3c) **(JP)** unused
    real, intent(inout) :: th0f(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: pi0f(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: dn0uf(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: dn0vf(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: zt(:)
    real, intent(inout) :: ztop  

    real :: c1,c2,b(1)
    integer :: i1,j1,i,j,k

    !     Interpolate the fine mesh 3-D reference state variables.

    if (icm == 0) return

!!$call fmint3(n1c,n2c,n3c,n1f,n2f,n3f,maxiz,maxix,maxiy  &
!!$     ,ifm,icm,nbot,ntop,jd,1,0,0,'t'  &
!!$     ,dn0c,dn0f,dn0c,dn0f,scr1,scr2,toptf,vt2da,b(1),b(1),b(1))
!!$
!!$call fmint3(n1c,n2c,n3c,n1f,n2f,n3f,maxiz,maxix,maxiy  &
!!$     ,ifm,icm,nbot,ntop,jd,1,0,0,'t'  &
!!$     ,th0c,th0f,dn0c,dn0f,scr1,scr2,toptf,vt2da,b(1),b(1),b(1))

    c1 = rgas / (cp - rgas)
    c2 = cp * (rgas / p00) ** c1
    pi0f(1:n1f,1:n2f,1:n3f) = c2 * (dn0f(1:n1f,1:n2f,1:n3f)  &
         *   th0f(1:n1f,1:n2f,1:n3f) ) ** c1


    call fillscr(1,maxix,maxiy,1,n2c,n3c,1,1,scr1,toptc)
!!$call eintp(scr1,scr2,1,maxix,maxiy,1,n2f,n3f,ifm,2,'t',0,0)
    call fillvar(1,maxix,maxiy,1,n2f,n3f,1,1,scr2,scr1)

    call rtgintrp_isan(n1f,n2f,n3f,th0f,scr1,toptf,zt,ztop)
    call rtgintrp_isan(n1f,n2f,n3f,pi0f,scr1,toptf,zt,ztop)

    ! Define dn0u and dn0v

    do j = 1,n3f
       j1 = min(j+1,n3f)
       do i = 1,n2f
          i1 = min(i+1,n2f)
          do k = 1,n1f
             dn0uf(k,i,j) = .5 * (dn0f(k,i,j) + dn0f(k,i1,j))
             dn0vf(k,i,j) = .5 * (dn0f(k,i,j) + dn0f(k,i,j1))
          enddo
       enddo
    enddo

    return
  end subroutine fmrefs3d_isan

  !*****************************************************************************

  subroutine fmdn0_isan(ifm,icm,n1f,n2f,n3f,n1c,n2c,n3c  &
       ,maxiz,maxix,maxiy,scr1,scr2,toptf,toptc,dn0f,dn0uf,dn0vf,zt,ztop)
    integer, intent(in) :: ifm
    integer, intent(in) :: icm
    integer, intent(in) :: n1f
    integer, intent(in) :: n2f
    integer, intent(in) :: n3f
    integer, intent(in) :: n1c
    integer, intent(in) :: n2c
    integer, intent(in) :: n3c
    integer, intent(in) :: maxiz
    integer, intent(in) :: maxix
    integer, intent(in) :: maxiy
    real, intent(inout) :: scr1(:)
    real, intent(in) :: scr2(:)
    real, intent(inout) :: toptf(:,:)  ! (n2f,n3f)
    real, intent(in) :: toptc(:,:) ! (n2c,n3c)
    real, intent(inout) :: dn0f(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: dn0uf(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: dn0vf(:,:,:) ! (n1f,n2f,n3f)
    real, intent(inout) :: zt(:)
    real, intent(inout) :: ztop  

    integer :: i,j,i1,j1,k
    !     Special vertical interpolation of DN0 must be done after all other
    !     3-D reference state and prognostic variables are interpolated.

    if (icm == 0) return

    call fillscr(1,maxix,maxiy,1,n2c,n3c,1,1,scr1,toptc)
!!$call eintp(scr1,scr2,1,maxix,maxiy,1,n2f,n3f,ifm,2,'t',0,0)
    call fillvar(1,maxix,maxiy,1,n2f,n3f,1,1,scr2,scr1)

    call rtgintrp_isan(n1f,n2f,n3f,dn0f,scr1,toptf,zt,ztop)

    ! Define dn0u and dn0v

    do j = 1,n3f
       j1 = min(j+1,n3f)
       do i = 1,n2f
          i1 = min(i+1,n2f)
          do k = 1,n1f
             dn0uf(k,i,j) = .5 * (dn0f(k,i,j) + dn0f(k,i1,j))
             dn0vf(k,i,j) = .5 * (dn0f(k,i,j) + dn0f(k,i,j1))
          enddo
       enddo
    enddo
  end subroutine fmdn0_isan

  !******************************************************************************

  subroutine rtgintrp_isan(n1,n2,n3,fld,vt2da,topt,zt,ztop)
    integer, intent(in) :: n1
    integer, intent(in) :: n2
    integer, intent(in) :: n3
    real, intent(inout) :: fld(:,:,:) ! (n1,n2,n3)
    real, intent(in) :: vt2da(n2,n3)
    real, intent(in) :: topt(:,:) ! (n2,n3)
    real, intent(in) :: zt(:) ! (n1)
    real, intent(in) :: ztop

    integer :: i,j,k
    real :: vctr1(n1)
    real :: vctr2(n1)
    real :: vctr3(n1)

    !     Do special vertical interpolation in case terrain on this grid
    !     (topt) is different from what would be interpolated from the
    !     coarser grid (vt2da).

    do j = 1,n3
       do i = 1,n2
          do k = 1,n1
             vctr1(k) = zt(k) * (1. - vt2da(i,j) / ztop) + vt2da(i,j)
             vctr2(k) = zt(k) * (1. - topt(i,j) / ztop) + topt(i,j)
             vctr3(k) = fld(k,i,j)
          enddo
          call htint(n1,vctr3,vctr1,n1,fld(1,i,j),vctr2)
       enddo
    enddo
  end subroutine rtgintrp_isan
  !     ****************************************************************

  subroutine varfile_refstate(n1,n2,n3,thp,pc,pi0,th0,rtp,dn0  &
       ,dn0u,dn0v,topt,rtgt,zt,ztop,piref,thref,dnref,rtref)
    integer, intent(in) :: n1
    integer, intent(in) :: n2
    integer, intent(in) :: n3
    real, intent(in) :: thp(:,:,:) ! (n1,n2,n3)
    real, intent(in) :: pc(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: pi0(:,:,:) ! (n1,n2,n3)
    real, intent(inout) :: th0(:,:,:) ! (n1,n2,n3)
    real, intent(in) :: rtp(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: dn0(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: dn0u(:,:,:) ! (n1,n2,n3)
    real, intent(out) :: dn0v(:,:,:) ! (n1,n2,n3)
    real, intent(in) :: topt(:,:) ! (n2,n3)
    real, intent(in) :: rtgt(:,:) ! (n2,n3)
    real, intent(in) :: zt(:) ! (n1)
    real, intent(in) :: ztop
    real, intent(out) :: piref(:) ! (n1)
    real, intent(out) :: thref(:) ! (n1)
    real, intent(out) :: rtref(:) ! (n1)
    real, intent(out) :: dnref(:) ! (n1)

    integer :: ir,jr,i,j,k,i1,j1
    real :: topref,c1,c2,c3
    real :: topu(n2,n3)
    real :: topv(n2,n3)
    real :: rtgu(n2,n3)
    real :: rtgv(n2,n3)
    real :: vctr1(n1)
    real :: vctr2(n1)

    !-srf - for reference state = mean of 2d fields
    integer ncount 
    real :: thp_m2d(n1)
    real :: rtp_m2d(n1)
    real :: pc_m2d(n1)
    integer, parameter :: ref_mean=0
    !--(DMK-CCATT-FIM)----------------------------------------------------------------

    !                Reference sounding is point with lowest topography

    topref=1.e10
    do j=1,n3
       do i=1,n2
          if(topt(i,j).lt.topref) then
             ir=i
             jr=j
             topref=topt(i,j)
          endif
       enddo
    enddo

    !--(DMK-CCATT-INI)----------------------------------------------------------------
    !srf- including Tremback's recommendation for iref,jref
    if(ccatt==1) then
       !- version 2 : ref == center of the domain
       ir=int(n2/2)
       jr=int(n3/2)
       topref=topt(ir,jr)
       print*,'Reference sounding is at=',ir,jr,topref
    endif
    !------


    !srf - version 3 - mean over horizontal model domain
    if(ref_mean == 1 ) then 
       thp_m2d=0.;rtp_m2d=0.; pc_m2d=0.

       do k = 1,n1
          ncount = 0
          do j=1,n3; do i=1,n2
             thp_m2d (k)= thp_m2d (k) + thp(k,i,j)
             rtp_m2d (k)= rtp_m2d (k) + rtp(k,i,j)
             pc_m2d  (k)= pc_m2d (k)  + pc(k,i,j)
             ncount = ncount +1
          enddo;enddo
          thp_m2d (k)= thp_m2d (k)/float(ncount)
          rtp_m2d (k)= rtp_m2d (k)/float(ncount)
          pc_m2d (k)= pc_m2d (k)/float(ncount)
       enddo
       topref=0.
       ncount=0    
       do j=1,n3; do i=1,n2
          topref=topref+topt(i,j)
          ncount = ncount +1
       enddo;enddo
       topref=topref/float(ncount)

    endif
    !------


    !srf : mean over horizontal model domain
    if(ref_mean == 1 ) then 

       do k=1,n1
          vctr2(k)=zt(k)*(1.-topref/ztop)+topref
       enddo

       call htint2(n1,thp_m2d,vctr2,n1,vctr1,zt)
       call htint2(n1,rtp_m2d,vctr2,n1,rtref(1),zt)

    else
       !--(DMK-CCATT-FIM)----------------------------------------------------------------

       do k=1,n1
          vctr2(k)=zt(k)*(1.-topref/ztop)+topref
       enddo

       call htint2(n1,thp(1,ir,jr),vctr2,n1,vctr1,zt)
       call htint2(n1,rtp(1,ir,jr),vctr2,n1,rtref(1),zt)

       !--(DMK-CCATT-INI)----------------------------------------------------------------
    endif
    !--(DMK-CCATT-FIM)----------------------------------------------------------------

    do k = 1,n1
       thref(k) = vctr1(k) * (1. + .61 * rtref(k))
    enddo
    rtref(1) = rtref(2)
    thref(1) = thref(2)

    !--(DMK-CCATT-INI)----------------------------------------------------------------
    !srf : mean over horizontal model domain
    if(ref_mean == 1 ) then 

       piref(1) =pc_m2d(1) + g * (vctr2(1) - zt(1))  &
            / (.5 * (thref(1)  &
            +  thp_m2d(1) * (1. + .61 * rtp_m2d(1))))

    else
       !--(DMK-CCATT-FIM)----------------------------------------------------------------

       piref(1) = pc(1,ir,jr) + g * (vctr2(1) - zt(1))  &
            / (.5 * (thref(1)  &
            + thp(1,ir,jr) * (1. + .61 * rtp(1,ir,jr))))
       !--(DMK-CCATT-INI)----------------------------------------------------------------
    endif
    !--(DMK-CCATT-FIM)----------------------------------------------------------------

    do k = 2,n1
       piref(k) = piref(k-1) - g * (zt(k)-zt(k-1))  &
            / ( .5 * (thref(k) + thref(k-1)))
    enddo

    do k = 1,n1
       vctr1(k) = (piref(k) / cp) ** cpor * p00
       dnref(k) = cp * vctr1(k)  &
            / (rgas * thref(k) * piref(k))
    enddo

    !        Compute 3-D reference state from 1-D reference state

    do j=1,n3
       do i=1,n2

          do k=1,n1
             vctr2(k)=zt(k)*rtgt(i,j)+topt(i,j)
          enddo
          call htint(n1,piref(1),zt,n1,pi0(1,i,j),vctr2)
          call htint(n1,thref(1),zt,n1,th0(1,i,j),vctr2)

          c1=g*2.*(1.-topt(i,j)/ztop)
          c2=(1-cpor)
          c3=cp**c2
          do k=n1-1,1,-1
             pi0(k,i,j)=pi0(k+1,i,j)  &
                  +c1*(zt(k+1)-zt(k))/(th0(k,i,j)+th0(k+1,i,j))
          enddo

          do k=1,n1
             dn0(k,i,j)=(c3*p00)/(rgas*th0(k,i,j)*pi0(k,i,j)**c2)
          enddo

       enddo
    enddo

    do j = 1,n3
       j1 = min(j+1,n3)
       do i = 1,n2
          i1 = min(i+1,n2)
          do k = 1,n1
             dn0u(k,i,j) = .5 * (dn0(k,i,j) + dn0(k,i1,j))
             dn0v(k,i,j) = .5 * (dn0(k,i,j) + dn0(k,i,j1))
          enddo
       enddo
    enddo
  end subroutine varfile_refstate
end module ModChemRefState
