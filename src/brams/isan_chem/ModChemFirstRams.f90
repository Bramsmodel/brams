...
contains
  ...
  subroutine first_RAMS(np1,np2,np3,ui2,vi2,pi2,ti2,ri2)
    ...
    real, intent(out) :: ui2(:,:,:) ! (np1,np2,np3)
    real, intent(out) :: vi2(:,:,:) ! (np1,np2,np3)
    real, intent(out) :: pi2(:,:,:) ! (np1,np2,np3)
    real, intent(out) :: ti2(:,:,:) ! (np1,np2,np3)
    real, intent(out) :: ri2(:,:,:) ! (np1,np2,np3)
...
!!$   ierr=RAMS_getvar('THETA',3,ngrid,ti2,rr_scr1,innpr(1:lenf))
       ierr=RAMS_getvar('RV', ngrid, ri2, rr_scr1, innpr)
       ierr=RAMS_getvar('PI', ngrid, pi2, rr_scr1, innpr)
!!$     ierr=RAMS_getvar('THETA',3,ngrid,ti2,rr_scr1,innpr(1:lenf))
       ierr=RAMS_getvar('THETA',ngrid,ti2,rr_scr1,innpr)
  ...
  subroutine isan_comp_dn0 (n1,n2,n3,pi0,th0,dn0,dn0u,dn0v,topt,ngrd) 
    ...
    do j=1,n3
       do i=1,n2
          ...
          enddo
          call htint(n1,piref(1,ngrd),ztn(1,ngrd),n1,vctr11,vctr2)
          call htint(n1,thref(1,ngrd),ztn(1,ngrd),n1,vctr12,vctr2)
...
!!$    real, intent(inout) :: dn0xc(:)
!!$    real, intent(inout) :: dn0xf(:)
    real, intent(inout) :: var1(:,:,:)
    real, intent(inout) :: var2(:,:,:)
    real, intent(inout) :: dn0xc(:,:,:)
    real, intent(inout) :: dn0xf(:,:,:)
