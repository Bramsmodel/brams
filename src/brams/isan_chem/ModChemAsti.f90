...
module ModChemAsti
  ...
  use isan_coms, only: &
       ...
       maxlev, maxsfc, maxsta, natime, nigrids, nisn, nprx, npry, nprz, &
       nsigz, nssfc, nsta, p_lat, p_lon, p_r, p_sfp, p_sft, p_slp, p_snow, &
       p_sst, p_t, p_u, p_ur, p_v, p_vr, p_z, pi_p, pi_r, pi_s, pi_u, pi_v, &
       pp_r, pp_sglob, pp_t, pp_u, pp_v, pp_z, ps_p, ps_r, ps_t, ps_u, &
       ps_v, rs_qual, rs_sfp, rs_sft, rs_slp, rs_snow, rs_sst, sf_chstid, &
       sf_date, sf_lat, sf_lon, sf_p, sf_r, sf_s, sf_scra, sf_t, sf_top, &
       sf_u, sf_ur, sf_v, sf_vr, up_chstid, up_lat, up_lon, up_lp, up_lz, &
       up_p, up_r, up_t, up_top, up_topg, up_ur, up_uz, up_vr, up_vz, up_z, &
       up_zz, upi_p, upi_r, upi_s, upi_u, upi_v, ups_p, ups_r, ups_t, &
       ups_u, ups_v, xswlat, xswlon
       ...
       if (guess1st=='PRESS') then
          ...
          if(CHEM_ASSIM == 1 .and. nspecies > 0) then
             ...
             do nspc=1,nspecies
                ...
                call vterpp_i (nnxp(ngrid),nnyp(ngrid),nprz,nisn  &
                     ,pp_u,pp_v,pp_t,pp_z,pp_sc(:,:,:,nspc)  &
                     ,pi_u,pi_v,pi_p,pi_s,pi_sc(:,:,:,nspc))
             enddo
          endif
          if(AER_ASSIM == 1 .and. nspecies_aer_in > 0) then
             !print*,'starting chemistry fields assimilation'
             do nspc=1,nspecies_aer_in
                ...
                !if(maxval(pp_sc(:,:,:,nspc)) < 1.e-20) stop 'wrong dpchem file'
                call vterpp_i (nnxp(ngrid),nnyp(ngrid),nprz,nisn  &
                     ,pp_u,pp_v,pp_t,pp_z,pp_aer_sc(:,:,:,nspc)  &
                     ,pi_u,pi_v,pi_p,pi_s,pi_aer_sc(:,:,:,nspc))
             enddo
          endif
          !srf-chem-end
          !--(DMK-CCATT-FIM)----------------------------------------------------------------
          ...
          if(CHEM_ASSIM == 1 .and. nspecies > 0) then
             do nspc=1,nspecies
                call vterpp_s (nnxp(ngrid),nnyp(ngrid),nprz,nsigz  &
                     ,pp_u,pp_v,pp_t,pp_z,pp_sc(:,:,:,nspc)   &
                     ,ps_u,ps_v,ps_p,ps_t,ps_sc(:,:,:,nspc)   &
                     ,grid_g(ngrid)%topt  &
                     ,grid_g(ngrid)%rtgt)
             enddo
          endif
          ...
          if(aer_ASSIM == 1 .and. nspecies_aer_in > 0) then
             do nspc=1,nspecies_aer_in
                call vterpp_s (nnxp(ngrid),nnyp(ngrid),nprz,nsigz  &
                     ,pp_u,pp_v,pp_t,pp_z,pp_aer_sc(:,:,:,nspc)   &
                     ,ps_u,ps_v,ps_p,ps_t,ps_aer_sc(:,:,:,nspc)   &
                     ,grid_g(ngrid)%topt  &
                     ,grid_g(ngrid)%rtgt)
             enddo
          endif
