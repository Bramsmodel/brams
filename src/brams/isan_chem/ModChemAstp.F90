...
module ModChemAstp
  ...
  use isan_coms, only: &
       PressLevs, ccGradsWrite, ccgradswrite, cntlat, cntlon, dlimit, &
       final_latitude, final_longitude, gdatdx, gdatdy, geo_varname, &
       icfiletype, icgradsprefix, idate, idatelin, idd, iglobew, iglobn, &
       iglobs, ihh, ihour, imm, imonth, initial_latitude, &
       initial_longitude, innpr, inproj, isversion, itinc, ivertcoord, &
       iyear, iyy, levpr, levpr_grib2, marker, mask, maxpr, ncVar, ncvar, &
       nprx, npry, nprz, nprz_grib2, nxGrib, nxgrib, nyGrib, nygrib, p_lat, &
       p_lon, p_r, p_sfp, p_sft, p_slp, p_snow, p_sst, p_t, p_u, p_ur, p_v, &
       p_vr, p_z, pnpr, pressLevs, presslevs, scale_factor, secondlat, &
       slevs, temperature_varname, ulimit, ur_varname, wind_u_varname, &
       wind_v_varname, xnelat, xnelon, xswlat, xswlon, z_max_level
  ...
  public :: chem_pressure_stage
  public :: chem_pressure_stage_netcdf
  public :: chem_pressure_stage_grib2
  public :: chem_pressure_stage_grads
...
contains
  ...
  subroutine chem_pressure_stage(n1,n2,nhem,glat,glon,glat2,glon2)
    ...
    real, intent(in) :: glat2(:,:) ! (n1,n2)
    real, intent(in) :: glon2(:,:) ! (n1,n2)
...
300 format(//,' Minimum THETA at ',I4,' mb - ',F8.2/  &
  ...
  subroutine chem_get_press_netCDF(uVarN,vVarN,tVarN,zVarN,qVarN &
       ,llat,llon,nTimes)
    integer, intent(in) :: uVarN
    integer, intent(in) :: vVarN
    integer, intent(in) :: tVarN
    integer, intent(in) :: zVarN
    integer, intent(in) :: qVarN
    real, intent(in) :: llat(:) ! (nyGrib)
    real, intent(in) :: llon(:) ! (nxGrib)
