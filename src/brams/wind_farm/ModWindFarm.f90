...
module ModWindFarm
  ...
  use module_wind_fitch, only: &
       dragforce, &
       init_module_wind_fitch, &
       piconst, &
       nt, &
       ival, &
       jval, &
  ...
  public :: windfarm
  public :: wind_farm_driver
  public :: StoreNamelistFileAtWindFarm
  public :: output_windFarms
...
contains
  ...
  subroutine wind_farm_driver(ngrid,m1,m2,m3,ia,iz,ja,jz, &
    ...
    call dragforce(                 & ! 01
         lid,                       & ! 02
         z_at_w,  & ! 03
         oneBasicFields%up, & ! 04
         oneBasicFields%vp, & ! 05
         grid_g(ngrid)%dxt, &
         grid_g(ngrid)%dyt, &
         dz,      & ! 07
         dtlongn(ngrid),            & ! 08
