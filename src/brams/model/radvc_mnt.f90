...
contains
  ...
  subroutine advmnt_driver(varn,m1 ,m2 ,m3 ,ia,iz,ja,jz,izu,jzv,mynum)
    ...
    if (dumpLocal) then
       ...
       call MsgDump(h//" partition global indices ("//&
       ...
       call MsgDump(h//" own (no ghost) global indices ("//&
            trim(adjustl(str(1)))//":"//trim(adjustl(str(2)))//","//&
            trim(adjustl(str(3)))//":"//trim(adjustl(str(4)))//")")
    end if
       ...
       if(use_true_density == OFF) then
          do ng=1,ngrids
             ...
             if (dumpLocal) then
                ...
                write(str(5),"(i8)") jEnd
                call MsgDump(h//&
                     " with use_true_density==0FF computes"//&
                     " iBegin="//trim(adjustl(str(2)))//&
                     ", iEnd="//trim(adjustl(str(3)))//&
                     ", jBegin="//trim(adjustl(str(4)))//&
                     ", jEnd="//trim(adjustl(str(5))))
             end if
    ...
    if (dumpLocal) then
       call MsgDump (h//" invokes prepare_winds")
    end if
    ...
    if (dumpLocal) then
       call MsgDump (h//" invokes get_Walceks_densities")
    end if
    ...
    if (dumpLocal) then
       call MsgDump (h//" invokes InitialFieldsUpdate twice")
    end if
  ...
  subroutine advect_mnt(ngrid,m1,m2,m3,ia,iz,ja,jz,dt,mynum,n,&
    ...
    !--- do X-advection
    if (dumpLocal) then
       call MsgDump(h//" update borders for x advection")
    end if
    ...
    if (dumpLocal) then
       call MsgDump(h//" invoke Advec3d_X")
    end if
    ...
    if (dumpLocal) then
       call MsgDump(h//" update borders for y advection")
    end if
    ...
    if (dumpLocal) then
       call MsgDump(h//" invoke Advec3d_Y")
    end if
    ...
    !--- do k-advection
    if (dumpLocal) then
       call MsgDump(h//" invoke Advec3d_Z")
    end if
  ...
  subroutine UpdateBorders(m1, m2, m3, field, &
    ...
    do iSend = 1, nSend
       ...
       if (dumpLocal) then
          write(str(1),"(i8)") iaSend(iSend)+i0
          write(str(2),"(i8)") izSend(iSend)+i0
          write(str(3),"(i8)") jaSend(iSend)+j0
          write(str(4),"(i8)") jzSend(iSend)+j0
          write(str(5),"(i8)") i2
          write(str(6),"(i8)") procSend(iSend)
          write(str(7),"(i8)") tagSend(iSend)
          call MsgDump(h//" dispatch send of global section ("//&
               trim(adjustl(str(1)))//":"//trim(adjustl(str(2)))//","//&
               trim(adjustl(str(3)))//":"//trim(adjustl(str(4)))//")"//&
    ...
    do iRecV = 1, nRecv
       ...
       if (dumpLocal) then
          write(str(1),"(i8)") iaRecv(recNum)+i0
          write(str(2),"(i8)") izRecv(recNum)+i0
          write(str(3),"(i8)") jaRecv(recNum)+j0
          write(str(4),"(i8)") jzRecv(recNum)+j0
          write(str(5),"(i8)") i2
          write(str(6),"(i8)") recNum
          write(str(7),"(i8)") tagRecv(recNum)
          call MsgDump(h//" recv #"//trim(adjustl(str(6)))//&
               " of tag "//trim(adjustl(str(7)))//&
               " with "//trim(adjustl(str(5)))//" reals and stores at global section ("//&
               trim(adjustl(str(1)))//":"//trim(adjustl(str(2)))//","//&
               trim(adjustl(str(3)))//":"//trim(adjustl(str(4)))//")")
       end if
    end do
