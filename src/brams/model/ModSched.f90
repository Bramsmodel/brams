...
contains
  ...
  subroutine schedule(isched, maxsched, maxschent, ngrids, nxtnest, nndtrat, nsubs)
    ...
    !
    ! schedule algorithm, giving the tree and the timestep ratio of each grid
    ...
    !
    ! schedule software assumptions: (not verified by modshched:)
  ...
  subroutine dump_schedule(isched, maxsched, maxschent, ngrids, nxtnest, nndtrat, nsubs)
    integer, intent(in) :: maxsched                   ! maximum number of grids in the tree of nested grids
    integer, intent(in) :: maxschent                  ! number of features per timestep stored at isched
    integer, intent(in) :: ngrids                     ! number of grids in the tree of nested grids
    integer, intent(in) :: nxtnest(ngrids)            ! parent grid
    ...
    character(len=*), parameter :: h="**(dump_schedule)**"
  ...
  subroutine cfll(n1,n2,n3,i0,j0,up,vp,wp,rtgt,f13t,f23t,dxt,dyt,&
             ...
             if ( cfl_sum > cfl_max_sum ) then
                ...
                if ( cfl_max_sum > 1.5 * 1.62 ) then
                   ...
                   call dump_cfl(cproc,n1,n2,n3,jdim,i0,j0,i,j,k,c1x,c1y,c1z,cfl_max_sum &
                        ...
                        ,wp(k,i,j),wp(k-1,i,j),dzt(k),oneBasicFields)
                endif
             end if
    ...
    if ( (dyncore_flag==0) .or. (dyncore_flag==1) ) then
       ...
       call fatal_error (h//" unknown dyncore_flag value!")
    end if
  ...
  subroutine dump_cfl(cproc,n1,n2,n3,jdim,i0,j0,i,j,k,c1x,c1y,c1z,cfl_max_sum &
    ...
    write(22,fmt='("PI0,Theta,thp,th0: ",4(E20.5,1X))') oneBasicFields%pi0(k,i,j),oneBasicFields%theta(k,i,j),oneBasicFields%thp(k,i,j),oneBasicFields%th0(k,i,j)
    write(22,*) "###########################################################################################################"
    close(unit=22)             
