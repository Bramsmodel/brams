...
module ModRadvcAdap
  ...
  public :: vel_advectc_adap
  public :: fa_preptc_adap
  public :: fa_xc_adap
  public :: fa_yc_adap
  public :: fa_zc_adap
  public :: advtndc_adap
...
contains
  ...
  subroutine vel_advectc_adap(m1,m2,m3,ia,iz,ja,jz,izu,jzv,jdim,lpu_R,lpv_R,lpw_R  &
    ...
    real, intent(in) :: aru(m1,m2,m3)
    real, intent(in) :: arv(m1,m2,m3)
    real, intent(in) :: arw(m1,m2,m3)
    real, intent(in) :: volu(m1,m2,m3)
    real, intent(in) :: volv(m1,m2,m3)
    real, intent(in) :: volw(m1,m2,m3)
    real, intent(inout) :: flxu(m1,m2,m3)
    real, intent(inout) :: flxv(m1,m2,m3)
    real, intent(inout) :: flxw(m1,m2,m3)
    real, intent(inout) :: ucb(m1,m2,m3)
    real, intent(inout) :: vcb(m1,m2,m3)
    real, intent(inout) :: wcb(m1,m2,m3)
    real, intent(inout) :: time !**(JP)** unused

    integer :: j,i,k,jm,im
    integer :: lpu(m2,m3),lpv(m2,m3),lpw(m2,m3)
    real :: fx1,fx2,fy1,fy2,fz1,fz2,cx1,cx2,cy1,cy2,cz1,cz2
  ...
  subroutine fa_preptc_adap(m1,m2,m3,vt3da,vt3db,vt3dc,vt3dd,vt3de,vt3df  &
    ...
    real, intent(in) :: arv(m1,m2,m3)
    real, intent(in) :: arw(m1,m2,m3)
    real, intent(in) :: volt(m1,m2,m3)
    real, intent(in) :: dxu(:,:) ! (m2,m3)
    real, intent(in) :: dyv(:,:) ! (m2,m3)
    real, intent(in) :: dxt(:,:) ! (m2,m3) !**(JP)** unused
    real, intent(in) :: dyt(:,:) ! (m2,m3) !**(JP)** unused
    real, intent(in) :: zt(:)
    real, intent(in) :: zm(:)
    real, intent(in) :: dzm(:)
    real, intent(out) :: vctr1(:)
    real, intent(out) :: vctr2(:)
    integer, intent(in) :: jd !**(JP)** unused
    integer, intent(in) :: mynum !**(JP)** unused
  ...
  subroutine fa_xc_adap(m1,m2,m3,ia,iz,ja,jz,lpw_R  &
    ...
    real, intent(inout) :: scr1(m1,m2,m3)
    real, intent(in) :: vt3da(m1,m2,m3)
    real, intent(in) :: vt3dd(m1,m2,m3)
    real, intent(inout) :: vt3dg(m1,m2,m3)
    real, intent(in) :: vt3dh(m1,m2,m3)
    integer, intent(in) :: mynum !**(JP)** unused
  ...
  subroutine fa_zc_adap(m1,m2,m3,ia,iz,ja,jz,lpw_R  &
    ...
    real, intent(in) :: scp(m1,m2,m3)
    real, intent(inout) :: scr1(m1,m2,m3)
    real, intent(in) :: vt3dc(m1,m2,m3)
    real, intent(in) :: vt3df(m1,m2,m3)
    real, intent(inout) :: vt3dg(m1,m2,m3)
    real, intent(in) :: vt3dh(m1,m2,m3)
  ...
  !     ****************************************************************
    ...
    do j = ja,jz
       do i = ia,iz
          do k = int(lpw_R(i,j)),m1-1
             sct(k,i,j) = sct(k,i,j) + (sca(k,i,j) - scp(k,i,j)) * dtli
          enddo
       enddo
    enddo
  end subroutine advtndc_adap
end module ModRadvcAdap
