...
contains
  ...
  subroutine aodDriver(m1,m2,m3,ia,iz,ja,jz,ngrids,&
    ...
    do k = 1,m1
       do i=ia,iz
          do j=ja,jz
             totm(i,j,k) = pmr(k,i,j)*oneBasicFields%dn0(k,i,j)
          enddo
       enddo
    enddo
  ...
  subroutine opt_read_aotmap(oneNamelistFile, oneBasicFields, oneTurbFields, gridId, oneControlVars)
    ...
    call parf_bcast(comm, int(size(comm,1),int64), master_num)
    ...
    call parf_bcast(infmapreal, int(size(infmap,1),int64), int(size(infmap,2),int64), master_num)
    !Make it int again
