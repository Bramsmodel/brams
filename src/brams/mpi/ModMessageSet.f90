module ModMessageSet
  ...
  public :: CreateMonAdvInputMessageSet
  public :: DestroyMonAdvInputMessageSet
...
contains
  ...
  function CreateMessageSet (name, direction, tag, hasMsg, Neigh) result(Msgs)
    ...
    if (ierr /= 0) then
       write(str(1),"(i8)") ierr
       write(str(2),"(i8)") nMsgs
       call fatal_error(h//" allocate Msgs%msgData("//trim(adjustl(str(2)))//&
    ...
    if (ierr /= 0) then
       write(str(1),"(i8)") ierr
       write(str(2),"(i8)") nMsgs
       call fatal_error(h//" allocate Msgs%request("//trim(adjustl(str(2)))//&
            ") fails with stat="//trim(adjustl(str(1))))
    end if
    ...
    if (ierr /= 0) then
       write(str(1),"(i8)") ierr
       write(str(2),"(i8)") nMsgs
       call fatal_error(h//" allocate Msgs%otherProc("//trim(adjustl(str(2)))//&
            ") fails with stat="//trim(adjustl(str(1))))
    end if
    ...
    if (cntMsg /= nMsgs) then
       write(str(1),"(i8)") cntMsg
       write(str(2),"(i8)") nMsgs
       call fatal_error(h//" inconsistency: cntMsg="//trim(adjustl(str(1)))//&
            " while nMsgs="//trim(adjustl(str(2))))
    end if
  ...
  subroutine DumpMessageSet(Msgs)
    ...
    if (.not. associated(Msgs)) then
       ...
       write(str(1),"(i8)") Msgs%nMsgs
       if (Msgs%tag == -1) then
          str(3)="UNDEF"
       else
          write(str(3),"(i8)") Msgs%tag
       end if
       call MsgDump(h//" "//trim(adjustl(Msgs%name))//&
            " has "//trim(adjustl(str(1)))//" messages and tag "//&
            trim(adjustl(str(3))))
       do i = 1, Msgs%nMsgs
          write(str(1),"(i8)") i
          if (Msgs%otherProc(i) == -1) then
             ...
             write(str(2),"(i8)") Msgs%otherProc(i)
          end if
          if (Msgs%request(i) == MPI_REQUEST_NULL) then
             str(3)="NULL"
          else
             write(str(3),"(Z8)") Msgs%request(i)
          end if
          call MsgDump(h//" message number "//trim(adjustl(str(1)))//&
               " to/from MPI node "//trim(adjustl(str(2)))//&
               ", has request "//trim(adjustl(str(3)))//&
               " and message data:")
       ...
       if (willComm(iNeigh)) then
          ...
          if (cntMsg > nMsgs) then
             ...
             call fatal_error(h//" nMsgs ("//&
                  trim(adjustl(str(1)))//") exceeded while inserting field "//&
          ...
          case (4,5)
             oneFieldSection =>  CreateFieldSection(&
                  ...
                  ybComm(iNeigh)-y0, yeComm(iNeigh)-y0)
          case default
             write(str(1),"(i8)") vTabPtr%idim_type
             call fatal_error(h//" unknown idim_type="//trim(adjustl(str(1)))//&
  ...
  subroutine InsertFieldSectionAtSendRecvMessageSetFromVTab(&
    ...
    call InsertFieldSectionAtMessageSetFromVTab(&
         myNum, vTabPtr, nNeigh, GlobalWithGhost, &
         xbSend, xeSend, ybSend, yeSend, willSend, SendMessageSet)
    call InsertFieldSectionAtMessageSetFromVTab(&
         myNum, vTabPtr, nNeigh, GlobalWithGhost, &
  ...
  subroutine CreateWideGhostZoneMessageSet(&
    ...
    integer, parameter :: mzi=-2
    integer, parameter :: mxi=-2
    integer, parameter :: myi=-2
    type(FieldSection), pointer :: oneFieldSection
...
!!$       end if
  ...
  subroutine CreateMonAdvInputMessageSet(&
       oneVarTable, oneVarTableSize, gridId, &
       GridSize, oneParallelEnvironment, Neigh, &
       GlobalOwnWithBC, GlobalWithGhost, &
       MonAdvInputSend, MonAdvInputRecv, TagMonAdvInput)
    ...
    type(DomainDecomp), pointer, intent(in) :: GlobalOwnWithBC
    type(DomainDecomp), pointer, intent(in) :: GlobalWithGhost
    type(MessageSet), pointer, intent(inout) :: MonAdvInputSend
    type(MessageSet), pointer, intent(inout) :: MonAdvInputRecv
    integer, intent(in) :: TagMonAdvInput
    ...
    character(len=*), parameter :: NameSendMonAdvInput="MonAdvInputSend"
    character(len=*), parameter :: NameRecvMonAdvInput="MonAdvInputRecv"

    character(len=*), parameter :: h="**(CreateMonAdvInputMessageSet)**"
    logical, parameter :: dumpLocal=.false.
    ...
    if (.not. associated(Neigh)) then
       MonAdvInputSend => null()
       MonAdvInputRecv => null()
       return
    end if

    if (dumpLocal) then
       call MsgDump(h//" will create MonAdvInputSend/Recv")
    end if
    ...
    ! MonAdvInputSend, MonAdvInputRecv:
    ! messages update entire GhostZone
    ...
    call NodesRegionsSendRecv(&
         ...
         Neigh=Neigh, &
         GlobalOwn=GlobalOwnWithBC, &
         NameSend=NameSendMonAdvInput, &
         NameRecv=NameRecvMonAdvInput, &
         xbToUpdate=GlobalWithGhost%xb, &
         xeToUpdate=GlobalWithGhost%xe, &
         ybToUpdate=GlobalWithGhost%yb, &
         yeToUpdate=GlobalWithGhost%ye, &
         ...
         yeSend=yeSend, &
         willSend=willSend, &
         SendMessageSet=MonAdvInputSend, &
         xbRecv=xbRecv, &
         xeRecv=xeRecv, &
         ybRecv=ybRecv, &
         yeRecv=yeRecv, &
         willRecv=willRecv, &
         RecvMessageSet=MonAdvInputRecv)

    ! take ModVarTables field that should be communicated
    ! (theta)

    call InsertFieldSectionAtSendRecvMessageSetFromVTab(&
         oneVarTable, oneVarTableSize, &
         "THETA", myNum, nNeigh, gridId, GlobalWithGhost, &
         xbSend, xeSend, ybSend, yeSend, willSend, MonAdvInputSend, &
         xbRecv, xeRecv, ybRecv, yeRecv, willRecv, MonAdvInputRecv)

    if (dumpLocal) then
       if (.not. associated(MonAdvInputSend)) then
          call MsgDump(h//" finishes empty MonAdvInputSend MessageSet")
       else
          call MsgDump(h//" finishes MonAdvInputSend MessageSet:")
          call DumpMessageSet(MonAdvInputSend)
       end if
       if (.not. associated(MonAdvInputRecv)) then
          call MsgDump(h//" finishes empty MonAdvInputRecv MessageSet")
       else
          call MsgDump(h//" finishes MonAdvInputRecv MessageSet:")
          call DumpMessageSet(MonAdvInputRecv)
       end if
    end if
  end subroutine CreateMonAdvInputMessageSet





  subroutine DestroyMonAdvInputMessageSet( &
       MonAdvInputSend, MonAdvInputRecv)

    type(MessageSet), pointer, intent(inout) :: MonAdvInputSend
    type(MessageSet), pointer, intent(inout) :: MonAdvInputRecv
    character(len=*), parameter :: h="**(DestroyMonAdvInputMessageSet)**"
    logical, parameter :: dumpLocal=.false.

    if (dumpLocal) then
       call MsgDump(h//" will destroy MonAdvInputSend/Recv")
    end if
    call DestroyMessageSet(MonAdvInputSend)
    call DestroyMessageSet(MonAdvInputRecv)

  end subroutine DestroyMonAdvInputMessageSet  
